package com.imagame.game 
{
	...
	public class AbstractPuzzle extends ImaSpriteGroup 
	{
		...
		protected var _dstRadio: Vector.<uint>;	//List of radio distances for all the pices, to check correcto drop position
		//mejor: vector de positiones y para cada uno lista de piezas (o de tipos de pieza) que acepta
		
		//TODO protected var _rounds: Vector.<uint>;		//List of initial round index in _dstlist vector. 0 if only one round. 
		...
		{ 			
			super(id);                         
			createPuzzleImage(pieces); //Create tilesheet and bmp images, and set x and y puzzle image position 
	...
	/*		s.dbgInit();
		 ...
		 */
		protected function createPuzzleImage(pieces: Vector.<Piece>):void { 
			var s:GameState = (Registry.game.getState() as GameState);
			_tileSheet = new ImaBitmapSheet(Assets.numberImages[s.phase-1], Assets.IMG_NUMBER_WIDTH, Assets.IMG_NUMBER_HEIGHT);
		...
		*/           
		protected function createAuxStructures(pieces: Vector.<Piece>):void { 
			...
			for(var i:uint; i<_numPieces; i++){ 
				_dstRadio[i] = pieces[i].w * 0.5; // 16;	//TODO
		...
		public function setPieceInPuzzle(piece: Piece, dstPos: Point):void { 
			...
			piece.playAnimation("InPuzzle", true, idx);	//utiliza idx para calcular frame, llamando a func pasad por parametro
...
/*
	...
	*/
		...
		*/ 
		...
		private function pointCloner(item:Point, index:int, vector:Vector.<Point>):Point {
			return item.clone();
		}
		private function local2global(item:Point, index:int, vector:Vector.<Point>):Point
		{
			item.offset(x, y);
			return item;
		} 
