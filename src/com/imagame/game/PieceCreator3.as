package com.imagame.game 
{
	...
	{		
		...
		private var _img: Bitmap;		//Bmp background image with the images of the level number (1..9)
		private var _gameState: ImaState;	//GameState that creates this object
		
		
		//specific piece creator variables
		private static const NUM_SHAPES: uint = 79; // 15 + 64; //Number of different shapes (mask pieces)
		private static const IDX_SHAPE_CONNECT: uint = 15; //Idx of the first connectable shape
		private var _shapeSize: Vector.<Point> = new Vector.<Point>(NUM_SHAPES);
		...
		var _groups: Vector.<uint> = new Vector.<uint>; 	//4 vals for each group: x,y, groupType, group_id within group type 
		  ...
		  * Creator of Puzzle-3
		  * @param	id	Number: 1..9
		  * @param	img	Base image composed of background + centered number img (required to obtain the pieces content)
		  * @return
		  */
		public function PieceCreator3(id: uint, img: Bitmap, gameState: ImaState) //TODO: color vector parameter
		{
			...
			//TODO create different size pieces. In v1.0 all have the same size
			//_shapeSize = new Vector.<Point>(NUM_SHAPES);
		//	for(var a:int=0; a< NUM_SHAPES; a++) 
		//		_shapeSize[a] = new Point(Assets.SPRITE_PIECESHAPE3_WIDTH, Assets.SPRITE_PIECESHAPE3_HEIGHT);
				
				
			//Shape groups definition
			//TODO: Automatic group definition (based on vars: area, connected, overlapped,..)
			_shapeGroup = new Array(9); //9 numbers 
			for(var j:uint=0; j< 9; j++) 
				_shapeGroup[j] = new Array(); 
			
			//Group Type 0: Groups of 1 shape => 3 groups	
			for (var k:uint = 0; k<NUM_SHAPES; k++)
				_shapeGroup[0].push(k, 0, 0); //shapeId, relative pos x, relative pos y
		...
		protected function setGroupsPieces():void { 
			...
			*/
			...
			switch(Registry.gpMgr['round']) {	
				case 0:
					numGroupsByLevel[0].push(5, 0, 0, 0, 0, 0, 0, 0, 0); //Num groups de nivel 1
					numGroupsByLevel[1].push(4, 1, 0, 0, 0, 0, 0, 0, 0); //Num groups de nivel 2
					numGroupsByLevel[2].push(4, 0, 1, 0, 0, 0, 0, 0, 0); //Num groups de nivel 3
					numGroupsByLevel[3].push(4, 0, 0, 1, 0, 0, 0, 0, 0); //Num groups de nivel 4
					numGroupsByLevel[4].push(4, 0, 0, 0, 1, 0, 0, 0, 0); //Num groups de nivel 5
					numGroupsByLevel[5].push(4, 0, 0, 0, 0, 1, 0, 0, 0); //Num groups de nivel 6
					numGroupsByLevel[6].push(5, 0, 0, 0, 0, 0, 1, 0, 0); //Num groups de nivel 7
					numGroupsByLevel[7].push(5, 0, 0, 0, 0, 0, 0, 1, 0); //Num groups de nivel 8
					numGroupsByLevel[8].push(5, 0, 0, 0, 0, 0, 0, 0, 1); //Num groups de nivel 9
					break;
				case 1:
					numGroupsByLevel[0].push(8, 0, 0, 0, 0, 0, 0, 0, 0); //Num groups de nivel 1
					numGroupsByLevel[1].push(6, 1, 0, 0, 0, 0, 0, 0, 0); //Num groups de nivel 2
					numGroupsByLevel[2].push(2, 2, 1, 0, 0, 0, 0, 0, 0); //Num groups de nivel 3
					numGroupsByLevel[3].push(2, 2, 0, 1, 0, 0, 0, 0, 0); //Num groups de nivel 4
					numGroupsByLevel[4].push(2, 1, 1, 0, 1, 0, 0, 0, 0); //Num groups de nivel 5
					numGroupsByLevel[5].push(2, 1, 1, 1, 0, 1, 0, 0, 0); //Num groups de nivel 6
					numGroupsByLevel[6].push(1, 2, 1, 1, 0, 0, 1, 0, 0); //Num groups de nivel 7
					numGroupsByLevel[7].push(1, 3, 1, 1, 0, 0, 0, 1, 0); //Num groups de nivel 8
					numGroupsByLevel[8].push(1, 3, 2, 1, 0, 0, 0, 0, 1); //Num groups de nivel 9
					break;					
				case 2:
					numGroupsByLevel[0].push(14, 0, 0, 0, 0, 0, 0, 0, 0); //Num groups de nivel 1
					numGroupsByLevel[1].push(10, 1, 0, 0, 0, 0, 0, 0, 0); //Num groups de nivel 2
					numGroupsByLevel[2].push(5, 4, 1, 0, 0, 0, 0, 0, 0); //Num groups de nivel 3
					numGroupsByLevel[3].push(4, 3, 0, 1, 0, 0, 0, 0, 0); //Num groups de nivel 4
					numGroupsByLevel[4].push(4, 2, 1, 0, 1, 0, 0, 0, 0); //Num groups de nivel 5
					numGroupsByLevel[5].push(4, 0, 2, 1, 0, 1, 0, 0, 0); //Num groups de nivel 6
					numGroupsByLevel[6].push(3, 0, 2, 1, 1, 0, 1, 0, 0); //Num groups de nivel 7
					numGroupsByLevel[7].push(3, 0, 2, 1, 0, 1, 0, 1, 0); //Num groups de nivel 8
					numGroupsByLevel[8].push(3, 0, 2, 1, 0, 0, 1, 0, 1); //Num groups de nivel 9
					break;					
				default: //rounds >=4
					numGroupsByLevel[0].push(20, 0, 0, 0, 0, 0, 0, 0, 0); //Num groups de nivel 1
					numGroupsByLevel[1].push(15, 1, 0, 0, 0, 0, 0, 0, 0); //Num groups de nivel 2
					numGroupsByLevel[2].push(10, 4, 1, 0, 0, 0, 0, 0, 0); //Num groups de nivel 3
					numGroupsByLevel[3].push(5, 3, 2, 1, 0, 0, 0, 0, 0); //Num groups de nivel 4
					numGroupsByLevel[4].push(5, 2, 2, 1, 1, 0, 0, 0, 0); //Num groups de nivel 5
					numGroupsByLevel[5].push(5, 3, 2, 1, 0, 1, 0, 0, 0); //Num groups de nivel 6
					numGroupsByLevel[6].push(3, 4, 2, 1, 1, 0, 1, 0, 0); //Num groups de nivel 7
					numGroupsByLevel[7].push(3, 4, 2, 1, 0, 1, 0, 1, 0); //Num groups de nivel 8
					numGroupsByLevel[8].push(3, 4, 2, 1, 0, 0, 1, 0, 1); //Num groups de nivel 9
			}
			...
			var zx:uint = (uint)((Registry.gameDefaultRect.width - Assets.IMG_NUMBER_WIDTH) * 0.5);	//480-192=288 / 2 = 144
			var zy:uint = (uint)(Registry.appUpOffset + 16); //				
			_auxRect.setTo(zx - 48, zy - 16, Registry.gameDefaultRect.width - (zx - 48) * 2, Assets.IMG_NUMBER_HEIGHT +  16 + 32); //96,24 288x272 
			...
			_auxRect.setTo(zx, zy, Registry.gameDefaultRect.width - zx * 2, Assets.IMG_NUMBER_HEIGHT); //144,48 192x224			
			...
			//pieces for level-number => Included total in number 
			//pieces for level-number -1 => Included partial around-number 
			//pieces for rest of levels => Excluded from around-number 
			...
			if (Registry.bAd) {
				...
				if(lvl == 0) {			
					//Put pieces of type-0: Only one piece equals to the level number, and the rest random figures (not numbers)
					grpMap.put(getSizeGroup(0,lvl),0,lvl,ImaRectAreaMap.TYPE_EXCLUDE_TOTAL,1,ImaRectAreaMap.TYPE_EXCLUDE_TOTAL,2,ImaRectAreaMap.TYPE_EXCLUDE_TOTAL,3, ImaRectAreaMap.TYPE_EXCLUDE_TOTAL, 4);
					for (var j:uint = 1; j < numGroupsByLevel[lvl][0]; j++) {
						val = ImaUtils.randomize(9, IDX_SHAPE_CONNECT-1); // _shapeGroupNumber[0]); //select a random shape (of type 0) bigger than a number shape
						grpMap.put(getSizeGroup(0, val), 0, val, ImaRectAreaMap.TYPE_EXCLUDE_TOTAL, 2, ImaRectAreaMap.TYPE_EXCLUDE_TOTAL, 3 , ImaRectAreaMap.TYPE_EXCLUDE_TOTAL, 4);					
					}
					...
					for (var j:uint = 1; j < numGroupsByLevel[lvl][0]; j++) {
						val = ImaUtils.randomize(9, IDX_SHAPE_CONNECT-1); // _shapeGroupNumber[0]); //select a random shape (of type 0) bigger than a number shape
						grpMap.put(getSizeGroup(0, val), 0, val, ImaRectAreaMap.TYPE_INCLUDE_PARTIAL, 0, ImaRectAreaMap.TYPE_EXCLUDE_TOTAL, 1,ImaRectAreaMap.TYPE_EXCLUDE_TOTAL, 2, ImaRectAreaMap.TYPE_EXCLUDE_TOTAL, 3, ImaRectAreaMap.TYPE_EXCLUDE_TOTAL, 4);
						//grpMap.put(getSizeGroup(0, val), 0, val, ImaRectAreaMap.TYPE_EXCLUDE_PARTIAL, 0, ImaRectAreaMap.TYPE_EXCLUDE_TOTAL,2,ImaRectAreaMap.TYPE_EXCLUDE_TOTAL,3);
					}	
					
					//Put pieces of type equal to the current level			
					val = ImaUtils.randomize(0, _shapeGroupNumber[lvl]); 
					if ( grpMap.put(getSizeGroup(lvl, val), lvl, val, ImaRectAreaMap.TYPE_INCLUDE_TOTAL, 1) == null)
						trace("ERR: grpMap.put not possible -> Time out");
					
					
					//Put pieces of type-(lvl-1) to type-1
					for (var i:uint = lvl - 1; i >= 1; i--) {
						var n:uint = numGroupsByLevel[lvl][i]; //Number of groups of type <i> to put in the map 
						for (var j:uint = 1; j <= n; j++) {  //For each group of type <i> 
							...
							grpMap.put(getSizeGroup(i,val),i,val,ImaRectAreaMap.TYPE_INCLUDE_PARTIAL,0,ImaRectAreaMap.TYPE_EXCLUDE_TOTAL,2,ImaRectAreaMap.TYPE_EXCLUDE_TOTAL,3, ImaRectAreaMap.TYPE_EXCLUDE_TOTAL, 4);              
						}
					}
				}
			...
			//No ads (only 3 zones, no banner zone)
			else {
				if(lvl == 0) {			
					//Put pieces of type-0: Only one piece equals to the level number, and the rest random figures (not numbers)
					grpMap.put(getSizeGroup(0,lvl),0,lvl,ImaRectAreaMap.TYPE_EXCLUDE_TOTAL,1,ImaRectAreaMap.TYPE_EXCLUDE_TOTAL,2,ImaRectAreaMap.TYPE_EXCLUDE_TOTAL,3);
					for (var j:uint = 1; j < numGroupsByLevel[lvl][0]; j++) {
						val = ImaUtils.randomize(9, IDX_SHAPE_CONNECT-1); // _shapeGroupNumber[0]); //select a random shape (of type 0) bigger than a number shape
					...
					for (var j:uint = 1; j < numGroupsByLevel[lvl][0]; j++) {
						val = ImaUtils.randomize(9, IDX_SHAPE_CONNECT-1); // _shapeGroupNumber[0]); //select a random shape (of type 0) bigger than a number shape
						grpMap.put(getSizeGroup(0, val), 0, val, ImaRectAreaMap.TYPE_INCLUDE_PARTIAL, 0, ImaRectAreaMap.TYPE_EXCLUDE_TOTAL, 1,ImaRectAreaMap.TYPE_EXCLUDE_TOTAL, 2, ImaRectAreaMap.TYPE_EXCLUDE_TOTAL, 3);
					}	
		...
		/*	lOGIC TO PROGRAMATICALLY INCREMENT THE NUMBERS OF SHAPES IN EACH ROUND
			//Put pieces of type-2 to type-9, except for the current level
			for(var i:uint=1; i<9; i++) { //for the rest of the group types 1..8 
				var n:uint = numGroupsByLevel[lvl][i]; //Number of groups of type <i> to put in the map 
				
				//Modulate n based on game global progress (game round)
				if (Registry.gpMgr['round'] > 0 ) {
					if (i < lvl)	
						n += Registry.gpMgr['round'];	//DIFFICULT LEVEL //n += (Registry.gpMgr as PropManager).round;				
				}
					
				
				for(var j:uint=1; j<=n; j++){  //For each group of type <i> 
					val = ImaUtils.randomize(0, _shapeGroupNumber[i]); //select a random shapeGroup id for the group type <i> 
					if(i == lvl) //if group type is the same as the current level => Shape group included Total in Number image 
									grpMap.put(getSizeGroup(i,val),i,val,ImaRectAreaMap.TYPE_INCLUDE_TOTAL,1);                                         
					else if (i <= lvl-1 && i > lvl-2) //caso lvls from [1..8]
									grpMap.put(getSizeGroup(i,val),i,val,ImaRectAreaMap.TYPE_INCLUDE_PARTIAL,0);                                         
					else if (i <= lvl-2 && i > lvl-3) //lvls from [2..8] 
									grpMap.put(getSizeGroup(i,val),i,val,ImaRectAreaMap.TYPE_INCLUDE_PARTIAL,0);                                         
					else if (i <= lvl-3 && i > lvl-4) //lvls from [3..8] 
									grpMap.put(getSizeGroup(i,val),i,val,ImaRectAreaMap.TYPE_INCLUDE_PARTIAL,0);                                         
					else if (i <= lvl-4 && i > lvl-5) //lvls from [4..8] 
									grpMap.put(getSizeGroup(i,val),i,val,ImaRectAreaMap.TYPE_INCLUDE_PARTIAL,0);                                         					
				}                                 
			}
		*/	
		...
		{			
			...
			//var abmdShapes:BitmapData = abmpShapes.bitmapData;        //Important: one asset for all shapes with the same size 
			...
			//var _bmpSheetFxPiece: ImaBitmapSheet = new ImaBitmapSheet(Assets.GfxIconsDlg, Assets.IMG_ICONDLG_WIDTH, Assets.IMG_ICONDLG_HEIGHT); 
...
/*		protected function mapPiecesToPositions():void {
			...
			for (var i:uint = 0; i < _numPieces; i++) {
				_dstMapPiece[i] = i;	//acceder al grupo, y dentro de este al idx_de la figura. Buscar su id de pieza
			}
		}*/
		...
		public function createPuzzle(): AbstractPuzzle {
			return (new Puzzle3(0, _pieces, _dstPos, _dstMapPiece));	//TODO PUZZLE2	
		}
